...
public class GamerResource {
    ...
    public ResponseEntity<Gamer> createGamer(@RequestBody Gamer gamer) throws URISyntaxException {
        ...
        if (gamer.getId() != null) {
            return ResponseEntity.badRequest().headers(HeaderUtil.createFailureAlert(ENTITY_NAME, "idexists", "A new gamer cannot already have an ID")).body(null);
        }
    ...
    @GetMapping("/gamers")
    @Timed
    public ResponseEntity<List<Gamer>> getAllGamers(@ApiParam Pageable pageable) {
