...
public class PronosticResource {
    ...
    public ResponseEntity<Pronostic> createPronostic(@RequestBody Pronostic pronostic) throws URISyntaxException {
        ...
        if (pronostic.getId() != null) {
            return ResponseEntity.badRequest().headers(HeaderUtil.createFailureAlert(ENTITY_NAME, "idexists", "A new pronostic cannot already have an ID")).body(null);
        }
    ...
    @Timed
    public ResponseEntity<List<Pronostic>> getAllPronostics(@ApiParam Pageable pageable) {
